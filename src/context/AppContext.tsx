...
import { DeviceAmount, type DeviceGet, type SupportCustomGet, type SupportParams, type SupportTypeGet, type SupportTypeParams, type SuppProductDetail } from "../Interface/SupportIn";
import { deleteSupportRequest, getDevicesAmountRequest, getSupportByParamsRequest, getSupportsCustomByFilterRequest, getSupportsCustomRequest, updateSupportTotalRequest } from "../services/Support.Service";
...
export const AppContexProvider = ({ children }: ContexArg) => {
    ...
    const [devicesAmount, setDevicesAmount] = useState<DeviceAmount[]>([])
    //abm
    ...
    function handleConfigSidebar(val: boolean) {
        setShowConfigSidebar(val)
    }
    function handleEnterpriseModal(val: boolean) {
        setShowEnterpriseModal(val)
    }
    async function listRevenues() {
        try {
            const res = await getRevenuesRequest()
    ...
    function changeProductAmount(id: number, amountCurrent: number) {
        ...
        const newProDetails: ProductDetail[] = productDetails.map((pro) => {
            if (pro.id == id) {
                pro.productAmount = amountCurrent
                pro.subtotal = pro.price * amountCurrent
            }
            return pro
        })
        // setProductDetails((detailPro) => {
        //     return detailPro.map((pro) => {
        //         return pro.id == id ? { ...pro, amount: 0+amountCurrent, subtotal: pro.price * amountCurrent } : pro
        //     })
        // })
        setProductDetails(newProDetails)
        sumTotal()
    }
    ...
    //support
    async function listDeviceAmount() {
        try {
            const res = await getDevicesAmountRequest()
            setDevicesAmount(res.data)
        } catch (error) {
            console.log(error)
        }
    }
    ...
    async function listSupportByFilter(str: string) {
        if (str == "") {
            listSupportType()
            return
        }
        ...
        try {
            const res = await getSupportsCustomByFilterRequest(str)
    ...
    async function listSupportByParams(params: SupportParams) {
        try {
            const res = await getSupportByParamsRequest(params)
            setSupports(res.data)
        } catch (error) {
            console.log(error)
        }
    ...
    return (
        <appContext.Provider value={{
            ...
            listSalesByDates, salesList, listSalesByParams, listSupportByParams, listDeviceAmount, devicesAmount,
