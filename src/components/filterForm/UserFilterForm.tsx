...
import type { UserParams } from "../../Interface/InAuth";

interface Prop{
    submitParam: (user: UserParams) => void
}

function UserFilterForm(prop: Prop) {

    const [propertyItems, _etPropertyItems] = useState<DropdownItem[]>([
        { label: "Nombre y Apellido", code: "name" },
        { label: "Nombre de usuario", code: "username" },
        { label: "Fecha Ingreso", code: "entry_date" },
    ]);
    const [propertySelect, setPropertySelect] = useState<DropdownItem | null>(null)
    const [checked, setChecked] = useState(false)

    const [roleItems, _setRileItems] = useState<DropdownItem[]>([
        { label: "ADMIN", code: "ADMININSTRADOR" },
        { label: "VENDEDOR", code: "VENDEDOR" },
        { label: "TECNICO", code: "TECNICO" },
    ]);
    const [roleSelect, setRoleSelect] = useState<DropdownItem | null>(null)
    ...
    function handleSubmit(){
        const userToSend: UserParams = {
            property: propertySelect?.code as string,
            role: roleSelect?.code as string,
    ...
    return (
        <>
            ...
            <div className="support-form-inputs">
                ...
                <div style={{ marginTop: "10px" }}>
                    <label htmlFor="username" style={{ marginTop: "10px" }}>Rol</label>
                    <Dropdown
                        variant="filled"
                        options={roleItems}
                        value={roleSelect}
                ...
                <div style={{ width: "440px", position: "absolute", bottom: "10px", right: "20px" }}>
                    <Button
