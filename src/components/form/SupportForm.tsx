...
import { customerListRequest, updateCustomerStatusRequest } from "../../services/Customer.service";
    ...
    async function updateSupport() {
        ...
        try {
            if (status == "CANCELADO" || status == "FINALIZADO") {
                ...
                await updateCustomerStatusRequest(
                    suppSend.customerId,
                    { name: "", lastname: "", telephone: "", document: "", address: "", status: false}
                )
                
                context.showToasSuccess("suporte "+status)
            } else {
                await updateCustomerStatusRequest(
                    suppSend.customerId,
                    { name: "", lastname: "", telephone: "", document: "", address: "", status: true}
                )
